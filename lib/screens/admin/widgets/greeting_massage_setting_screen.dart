...
class _AdminGreetingSettingScreenState
  ...
  bool _isFormValid() {
    return _greetingTextController.text.trim().isNotEmpty || _hideGreeting;
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // 제목 섹션 (환영메시지 숨기기 체크 시 미리보기에서도 미표시)
                ...
                _isFormValid()
                    ? () async {
                      ...
                      try {
                        ...
                        // 플레이스 등록 직후에는 AppStartup(/)을 거치지 않고 관리자 홈으로 바로 이동
                        // (AppStartup의 requireManual 등으로 인해 waiting으로 튕기는 것 방지)
                        if (!mounted) return;
                        Navigator.of(
                          context,
